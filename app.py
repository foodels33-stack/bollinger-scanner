...
def send_telegram(msg, specific_chat_id=None):
    try:
        token = str(st.secrets.get("BOT_TOKEN", BOT_TOKEN)).strip()
        targets = [specific_chat_id] if specific_chat_id else get_all_chat_ids()
        ok=False
        for chat_id in targets:
            url = f"https://api.telegram.org/bot{token}/sendMessage"
            r = requests.get(url, params={"chat_id": str(chat_id).strip(), "text": msg}, timeout=15)
...
def run_scan_20():
    ...
    for t in TURBO_LIST:
        ...
        try:
            ...
            curr_close = float(l["Close"])
            chg = (prev_close-curr_close)/prev_close*100 if prev_close!=0 else 0
            if chg <= 0.3: continue
            vr=l["Volume"]/l["VOL_AVG"] if l["VOL_AVG"]>0 else 0
            r=analyze_long(d)
            if r["score"]>=80:
                tp_mid = round(float(l["BB_M"]),2)
                profit = round((tp_mid - float(l["Close"]))/float(l["Close"])*100,2) if float(l["Close"])!=0 else 0
...
with c3:
    period=st.selectbox("תקופה",["1mo","3mo","6mo","1y","5d"], index=0)
with c4:
...
with c7:
    auto_scan=st.toggle("🤖 אוטומט + טלגרם", value=False, disabled=not st.session_state.is_admin)
with c8:
    if st.session_state.is_admin:
        ...
        st.button("📩 טסט טלגרם", disabled=True)

if ticker:
    df=get_data(ticker,period,interval)
    if df.empty:
        st.warning(f"אין נתונים ל-{ticker} עם {period}/{interval}.")
    elif len(df)<20:
        st.warning(f"יש רק {len(df)} נרות ל-{ticker}. צריך לפחות 20. בחר תקופה גדולה יותר כמו 1mo.")
        ...
        if st.button("Run Now - מצא נפילות לקנייה", use_container_width=True):
            with st.spinner("סורק 80 טיקרים לנפילות..."):
                ...
                if results:
                    ...
                    st.info("לא נמצאו נפילות כרגע")
    else:
        st.button("Run Now - מצא נפילות לקנייה", use_container_width=True, disabled=True)
        st.caption("רק מנהל יכול להריץ סריקה")
with col_b:
    st.caption(f"אוטומט סורק כל 2 דקות ושולח ל-{BOT_USERNAME}")
    ...
    try:
        ...
        with st.spinner("סריקה אוטומטית רצה..."):
            ...
            if results:
                ...
                if not prev or (len(prev)>0 and results[0]["SYMBOL"]!= prev[0]["SYMBOL"]):
                    st.session_state.scan_results=results
...
if results:
    ...
    st.info("לחץ Run Now או הפעל אוטומט (מנהל בלבד)")
