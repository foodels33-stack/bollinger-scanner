...
def make_chart(ticker, tf):
    try:
        ...
        fig.update_layout(height=700, template="plotly_dark", paper_bgcolor="#111315", plot_bgcolor="#1a1d22", xaxis_rangeslider_visible=False, font=dict(color="#ffffff", size=13), title=dict(text=f"{ticker} ${p:.2f} | {now_disp} | {tf} | {wr}% | {market_status} | TURBO", font=dict(size=14, color="#00ff88")), margin=dict(l=10,r=10,t=60,b=10))
...
def run_scan(n, mode, tf):
    ...
    for i,sym in enumerate(syms[:n]):
        status.text(f"סורק {i+1}/{len(syms[:n])} : {sym} | {datetime.now(IL_TZ).strftime('%H:%M:%S')} | {mode}")
        try:
            ...
            if mode=="NASDAQ 100 TURBO 🚀":
                if not is_break:
                    if not (p>l and float(d['Close'].iloc[-2]) < float(d['lower'].iloc[-2]) and v>vm*1.3):
                        continue
            else:
            ...
            if enable_tg and (is_extreme or (mode=="NASDAQ 100 TURBO 🚀" and is_break)):
                send_telegram(f"🚀 {sym} {sig} ${p:.2f} | RSI {r:.0f} | Vol x{v/vm:.1f} | {btime}")
        except:
            pass
    ...
    status.empty()
    st.session_state.is_scanning=False
    st.success(f"סריקה {mode} הושלמה! נמצאו {len(res)} | {datetime.now(IL_TZ).strftime('%H:%M:%S')}")
...
with right:
    st.markdown("### ⚙️ סריקה")
    st.caption(f"{market_status} | {datetime.now(IL_TZ).strftime('%H:%M:%S')} | BOT: @omer_turbo72_bot")
    mode=st.selectbox("סוג סינון", ["NASDAQ 100 TURBO 🚀","הכל","קיצוני 🚨","רותחות 🔥","בולינגר"])
    colA,colB=st.columns(2)
    with colA:
        if st.button("SCAN 100 TURBO", use_container_width=True, type="primary"):
...
with left:
    ...
    if fig:
        if wr>=65:
            st.success(f"🔴 לייב: {st.session_state.focus} | {live_time} | הצלחה {wr}% | ✅ קנה | TURBO")
        else:
